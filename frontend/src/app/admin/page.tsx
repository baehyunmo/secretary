  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={s.label} className="bg-white rounded-2xl p-4 shadow-sm text-center">
              <div className="text-3xl font-bold text-indigo-500">{s.val}</div>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm">
        <h2 className="text-lg font-semibold mb-4 pb-3 border-b">새 임원 등록</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">이름</label>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="홍길동" className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">직위</label>
            <select value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm">
              {POSITIONS.map((p) => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">부서</label>
            <input value={form.dept} onChange={(e) => setForm({ ...form, dept: e.target.value })} placeholder="경영기획실" className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">연락처</label>
            <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="010-0000-0000" className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">이메일</label>
            <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="example@company.com" className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">비고</label>
            <input value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="특이사항..." className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
          </div>
        </div>
        <button onClick={addExec} className="bg-indigo-500 text-white px-5 py-2 rounded-lg text-sm hover:bg-indigo-600">임원 등록</button>
      </div>

      {/* 임원 목록 */}
      <div className="bg-white rounded-2xl p-5 shadow-sm">
        <h2 className="text-lg font-semibold mb-4 pb-3 border-b">임원 목록</h2>
        <div className="flex gap-3 mb-4">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="이름, 직위, 부서 검색..." className="flex-1 px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
          <select value={filterPos} onChange={(e) => setFilterPos(e.target.value)} className="px-3 py-2 border-2 border-gray-200 rounded-lg text-sm">
        ...
        {filtered.length === 0 ? (
          <p className="text-gray-400 text-center py-6">등록된 임원이 없습니다.</p>
        ) : (
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} value={(form as any)[f.key]} onChange={(e) => setForm({ ...form, [f.key]: e.target.value })} className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
                </div>
              ))}
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1">직위</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1">비고</label>
                <textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} rows={3} className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm" />
              </div>
            </div>
            <div className="flex gap-2 mt-5">
              <button onClick={saveEdit} className="bg-indigo-500 text-white px-5 py-2 rounded-lg text-sm">저장</button>
              <button onClick={() => setShowModal(false)} className="border-2 border-indigo-500 text-indigo-500 px-5 py-2 rounded-lg text-sm">취소</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* 상세 모달 */}
      {showDetail && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowDetail(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4 text-sm">
              ...
              <div><b>이메일:</b> {showDetail.email || "-"}</div>
            </div>
            ...
            {(showDetail.trips?.length || 0) === 0 ? (
              ...
              <div className="space-y-1">
                {showDetail.trips.map((t: any) => (
                  <div key={t.id} className="text-sm py-1 border-b border-gray-50">
            ...
            <div className="mt-4">
              <button onClick={() => setShowDetail(null)} className="border-2 border-indigo-500 text-indigo-500 px-5 py-2 rounded-lg text-sm">닫기</button>
            </div>
          </div>
        </div>
      )}
